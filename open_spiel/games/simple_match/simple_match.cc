...
std::vector<CellState> SimpleMatchState::Board() const {
  std::vector<CellState> board(board_.begin(), board_.end());
  return board;
}


void SimpleMatchState::DoApplyAction(Action move) {
...
bool SimpleMatchState::HasLine(Player player) const {
  return BoardHasLine(board_, player);
}

bool SimpleMatchState::IsFull() const { return num_moves_ == kNumCells; }
